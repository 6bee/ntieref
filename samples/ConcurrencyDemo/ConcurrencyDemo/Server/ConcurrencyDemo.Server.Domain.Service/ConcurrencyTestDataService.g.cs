    ...
    public partial class ConcurrencyTestDataService : DataService<IConcurrencyTestRepository>, IConcurrencyTestDataService
    {
        ...
        public QueryResult<ARecord> GetARecords(ClientInfo clientInfo, Query query)
        {
            using (var dataRepository = _repositoryFactory(clientInfo))
            {
                ...
                var result = Get(dataRepository.ARecords.AsNoTrackingQueryable(), query, clientInfo);
        ...
        public QueryResult<BRecord> GetBRecords(ClientInfo clientInfo, Query query)
        {
            ...
            {
                PreProcessing(clientInfo, ref query, dataRepository);
                var result = Get(dataRepository.BRecords.AsNoTrackingQueryable(), query, clientInfo);
                PostProcessing(clientInfo, query, ref result, dataRepository);
                return result;
            }
        }
        ...
        public QueryResult<CRecord> GetCRecords(ClientInfo clientInfo, Query query)
        {
            ...
            {
                PreProcessing(clientInfo, ref query, dataRepository);
                var result = Get(dataRepository.CRecords.AsNoTrackingQueryable(), query, clientInfo);
                PostProcessing(clientInfo, query, ref result, dataRepository);
                return result;
            }
        }
        ...
        {
            return new FaultException<ConcurrencyTestUpdateFault>(new ConcurrencyTestUpdateFault(message, entities), "Update error");
        }

        protected override FaultException CreateOptimisticConcurrencyFaultException(string message, IEnumerable<Entity> entities)
        {
            return new FaultException<ConcurrencyTestOptimisticConcurrencyFault>(new ConcurrencyTestOptimisticConcurrencyFault(message, entities), "Optimistic concurrency error");
        }
