...
namespace NTier.Server.Domain.Repositories.EntityFramework
{
    internal static class SelfTrackingEntitiesContextExtensions
    {
        ...
        //cse: removed
        //// christof.senn@trivadis.com 
        //private static readonly Regex IsNotNullPattern = new Regex(@"(?<is_not_null>[\s]+!=[\s]+null)(?![A-Za-z0-9_])", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        //private static readonly Regex IsNullPattern = new Regex(@"(?<is_null>[\s]+==[\s]+null)(?![A-Za-z0-9_])", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        //private static readonly Regex AndPattern = new Regex(@"(?<and>([\s]*&&[\s]*)|([\s]+AndAlso[\s]+))", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        //private static readonly Regex OrPattern = new Regex(@"(?<or>([\s]*\|\|[\s]*)|([\s]+OrElse[\s]+))", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        ////private static readonly Regex BitwiseAndPattern = new Regex(@"(?<and>([\s]*&[\s]*)|([\s]+And[\s]+))", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        ////private static readonly Regex BitwiseOrPattern = new Regex(@"(?<or>([\s]*\|[\s]*)|([\s]+Or[\s]+))", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        //private static readonly Regex IsNotEqualPattern = new Regex(@"(?<is_not_equal>[\s]+!=[\s]+|[\s]+<>[\s]+)", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);
        ////private static readonly Regex ParameterNamePattern = new Regex(@"(?<=(^|[\s]*))(?<parameter>\$)(?=\.)", RegexOptions.Compiled | RegexOptions.ExplicitCapture | RegexOptions.IgnoreCase);

        //private static string Replace(this string input, Regex regex, string replacement)
        //{
        //    return regex.Replace(input, replacement);
        //}

        //cse: removed
        //// Applies the query proeprties to the object query
        //public static IQueryable<TEntity> ApplyQuery<TEntity>(this ObjectQuery<TEntity> objectQuery, Query query, bool isCountQuery = false) where TEntity : class
        //{
        //    // ObjectQuery<>

        //    // include
        //    if (!isCountQuery && query.Include != null)
        //    {
        ...
        //    {
        //        var filterString = query.Filter.Value
        //            .Replace("$.", "it.")
        //            .Replace("\"", "'")
        //            .Replace(OrPattern, " OR ")
        //            .Replace(AndPattern, " AND ")
        //            .Replace(IsNullPattern, " IS NULL")
        //            .Replace(IsNotNullPattern, " IS NOT NULL")
        //            .Replace(IsNotEqualPattern, " <> ")
        //            //.Replace(BitwiseAndPattern, " & ")
        //            //.Replace(BitwiseOrPattern, " | ")
        //            ;

        //        objectQuery = objectQuery.Where(filterString, query.Filter.Parameters.ToObjectParameters());
        //    }

        //    // sorting
        //    if (!isCountQuery && query.OrderBy != null)
        //    {
        //        var orderString = query.OrderBy.Value.Replace("$.", "it.");
        //        objectQuery = objectQuery.OrderBy(orderString, query.OrderBy.Parameters.ToObjectParameters());
        //    }

        //    // IQueryable<>
        //    IQueryable<TEntity> resultQuery = objectQuery;
        ...
        //        {
        //            resultQuery = resultQuery.Skip(query.Skip.Value);
        //        }

        //        if (query.Take.HasValue)
        //        {
        //            resultQuery = resultQuery.Take(query.Take.Value);
        //        }
        //    }
        ...
        //cse: removed
        //private static ObjectParameter[] ToObjectParameters(this IEnumerable<Parameter> parameters)
        //{
        //    return parameters == null ? new ObjectParameter[0] : parameters.Select(p => new ObjectParameter(p.Name, p.Value)).ToArray();
        //}
    }
}